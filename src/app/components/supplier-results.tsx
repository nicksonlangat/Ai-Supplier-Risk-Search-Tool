...
export function SupplierResults({ suppliers }: SupplierResultsProps) {
  if (!suppliers || suppliers.length === 0) {
    return <div className="text-gray-500">No suppliers found matching your criteria.</div>;
  }
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Risk Categories</th>
          </tr>
        </thead>
        <tbody className="bg-white divide-y divide-gray-200">
          {suppliers.map((supplier) => (
            <tr key={supplier.id}>
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{supplier.industry}</td>
